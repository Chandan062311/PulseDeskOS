...
This changelog describes the shipped public surface. Detailed implementation
boundaries live in `ARCHITECTURE.md`; runnable checks live in `tests/`.

## v1.0 — 2026-09-20

Release hardening + documentation freeze.

- Full pipeline: triage (9 questions:
...
- Regression hardening: tenant-scoped delete; `Frozen`
...
- Verification is automated through CI with pytest, ruff, mypy, plugin manifest
  checks, secret scanning, and a frontend production build.
...
- Initial triage + ingest API with config-gated actions and the
