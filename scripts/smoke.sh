...
# Auth: servers with PULSEDESK_API_KEY set gate /v1/* via X-API-Key.
AUTH=()
if [ -n "${PULSEDESK_API_KEY:-}" ]; then AUTH=(-H "X-API-Key: $PULSEDESK_API_KEY"); fi
...
count_body="$(curl -s "${AUTH[@]}" "$API/v1/memory/count?tenant_id=smoke")"
...
# 5. Auth gate (only when server is keyed)
if [ -n "${PULSEDESK_API_KEY:-}" ]; then
  gate_code="$(code_of "$API/v1/triage" -X POST -H 'Content-Type: application/json' -d '{"ticket":{"subject":"s","message":"m"}}')"
  if [ "$gate_code" = "401" ]; then
    pass "auth gate -> 401 without key"
  else
    fail "auth gate -> $gate_code (want 401 without key)"
  fi
fi

# 6. Cleanup smoke rows (best-effort)
if [ -n "${store_json:-}" ]; then
  smoke_id="$(printf '%s' "$store_json" | grep -o '"id"[[:space:]]*:[[:space:]]*"[^"]*"' | head -n1 | cut -d'"' -f4)"
  if [ -n "${smoke_id:-}" ]; then
    curl -s -o /dev/null "${AUTH[@]}" -X DELETE "$API/v1/memory/$smoke_id?tenant_id=smoke" || true
  fi
fi
